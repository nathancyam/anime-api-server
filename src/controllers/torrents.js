...
function TorrentRouter(loggedInMiddleware, apiMiddleware) {
   ...
   * @param res
   */
  router.post('/add', loggedInMiddleware, (req, res) => {
  ...
  router.post('/server', apiMiddleware, (req, res) => {
  ...
  router.post('/server/update', loggedInMiddleware, (req, res) => {
  ...
  router.post('/move/:torrentId/anime/:animeId', loggedInMiddleware, (req, res) => {
    const torrentServer = req.app.get('torrent_server');
  ...
  router.post('/pause/:torrentId', loggedInMiddleware, (req, res) => {
    req.app.get('torrent_server')
      .pauseTorrent(req.params.torrentId)
      .then(response => res.json(response));
  });

  router.post('/resume/:torrentId', loggedInMiddleware, (req, res) => {
    req.app.get('torrent_server')
